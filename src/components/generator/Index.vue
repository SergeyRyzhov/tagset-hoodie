  ...
  <b-container fluid>
    ...
    <b-form-group description="Select topics which describe your set">
      <b-button
        ...
        :key="'topic' + topic._id"
        size="sm"
        :variant="!selectedTopics.hasOwnProperty(topic._id) ? 'primary' : 'warning'"
    ...
    <b-form-group description="Configure quality of tags which you want to receive">
      ...
      <b-form-input
        v-model="goal"
        type="number"
        placeholder="Enter your goal count"
      />
      <b-button
        variant="success"
        right
    ...
    <b-alert
      show
      :variant="improvements.alertType"
    >
      Current set: {{ score.amount }} tags with average rate {{ score.average | float }} ({{ improvements.delta | float }})
    </b-alert>

    <b-form-group description="Correct as you wish">
      <div
        v-for="topic in selectedTopics"
        :key="'s-topic-tags' + topic._id"
      >
        <b-button
          variant="danger"
          size="sm"
        ...
        <div
          v-for=" tag in tagsOfTopic(topic)"
          :key="'tag' + tag._id"
        ...
        >
          <b-button
            ...
            size="sm"
            :pressed="selectedTags.hasOwnProperty(tag._id)"
            @click="toggleTag(tag, topic)"
          >
            {{ tag.title }}
          </b-button>
    ...
    <b-form-group description="Review and take yout set">
      <b-textarea
        ...
        placeholder="Your set will be here"
        readonly="readonly"
        :rows="4"
        :min-rows="1"
        :max-rows="7"
      ...
      <b-button
        variant="success"
        size="sm"
        :disabled="combinedTags.length == 0"
        @click="toBuffer"
      >
        Copy to buffer
      </b-button>
...
export default {
  ...
  computed: {
    ...
    score () {
      ...
      var average = selectedTags.reduce((sum, tag) => {
        sum += tag.rate / amount
        return sum
      }, 0)
      return {
  ...
  methods: {
    ...
    selectBest () {
      var allTags = Object.values(this.selectedTopics).reduce((tags, topic) => {
        tags.push(this.tagsOfTopic(topic))
        return tags
      }, [])
    ...
    combine () {
      ...
      let combinedTags = tags.map(tag => '#' + tag.title).join(' ')
